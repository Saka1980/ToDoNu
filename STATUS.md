...
- ✅ **Dra-och-släpp-ordning på projekt** (VER 1.10) — varje projektkort har en greppikon `≡`; håll och dra i den för att flytta projektet upp/ned i listan. Tryck på resten av kortet öppnar projektet som vanligt. Touch-baserat (Pointer Events, fungerar på Pixel + mus), ingen datamodell-ändring (ordningen ligger i `s.projects`-arrayen). `setupProjectDrag()`/`gripDown/Move/Up()`/`dragAfter()` i `index.html`. `sw.js`-cache → `todonu-v4`.
