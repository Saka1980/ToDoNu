...
- ✅ **Dra-och-släpp-ordning på projekt** (VER 1.11) — varje projektkort har en greppikon `≡`; håll och dra i den för att flytta projektet upp/ned i listan. Tryck på resten av kortet öppnar projektet som vanligt. Touch-baserat (Pointer Events, fungerar på Pixel + mus), ingen datamodell-ändring (ordningen ligger i `s.projects`-arrayen). Draget sker via `transform` (kortet följer fingret, grannarna glider) och sorterar om arrayen vid släpp — **inte** via DOM-flytt under draget (det avbröt Chrome:s pointer-capture, v1.10-buggen). `setupProjectDrag()`/`gripDown/Move/Up()`/`shiftNeighbors()` i `index.html`. `sw.js`-cache → `todonu-v5`.
